...
from reacnetgenerator._detect import _DetectCrd, _DetectLAMMPSdump
...
class TestASEMode:
    ...
    def test_ase_duplicate_bond_prevention(self):
        ...
        for i, neighbors in enumerate(bond):
            ...
            assert len(neighbors) == len(set(neighbors)), f"Duplicate bonds found for atom {i}"
            # Check that the corresponding bond levels are also unique
            assert len(bondlevel[i]) == len(set(bondlevel[i])) or len(bondlevel[i]) == len(neighbors)

    @pytest.mark.skipif(not (ASE_AVAILABLE and SCIPY_AVAILABLE), reason="ASE or Scipy not available")
...
""")
    ...
    def test_ase_warning_on_auto_enable(self, caplog):
        ...
        with caplog.at_level("WARNING"):
            ...
            assert "ASE parameters detected. Automatically enabling --use-ase mode." in caplog.text
            assert rng.use_ase